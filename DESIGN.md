...
reconciliation step that runs at deterministic command boundaries:

1. Run reconciliation at the start of `codexauth list` (before usage refresh)
   so external updates are captured before any in-app refresh/write behavior.
2. Run reconciliation immediately before replacing `~/.codex/auth.json` during
   `codexauth use` (or interactive activation from `list`).
3. Other commands may skip auto-reconciliation to avoid surprising writes during
   read-only or setup operations.

Reconciliation behavior for each run:
...
### Reconciliation Timing Decision

The preferred default is **not** "on every app run." Instead, run reconciliation
on `list` and `use` only:

- `list` is the highest-value read path because it can trigger usage refreshes;
  reconciling first prevents stale-store refresh decisions.
- `use` is the highest-risk write path because it can overwrite
  `~/.codex/auth.json`; reconciling first preserves external token updates from
  the currently active profile before switching away.
- Running on every command invocation would increase hidden side effects for
  commands such as `status`, `help`, and config/setup flows where users do not
  expect store writes.

Future extension: add a global opt-in mode (for example `CODEXAUTH_RECONCILE_ALWAYS=1`)
for operators who explicitly want reconciliation on every CLI entrypoint.
