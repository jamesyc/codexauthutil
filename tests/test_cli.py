...
def test_start_weekly_includes_zero_percent_placeholder(
    ...
    assert "weekly timer unverified" in result.output
...
def test_use_push_prompt_reprompts_on_empty_or_q(runner, monkeypatch, tmp_path):
    ...
    assert result.exit_code == 0
    assert result.output.count("Reconciliation updated local store. Sync these changes now? [y/N]:") == 3
    assert pushed == [sync_dir]
...
def test_list_hides_spark_columns_by_default_and_shows_them_with_all(
    runner, saved_profile, monkeypatch
):
    async def fake_fetch_all_usage(profiles):
        return usage_module.UsageFetchSummary(
            usage_map={
                "work": cli_module.UsageResult(
                    windows={
                        ...
                        "additional_gpt_5_3_codex_spark_primary_window": usage_module.UsageWindow(
                            key="additional_gpt_5_3_codex_spark_primary_window",
                            used_pct=12,
                            label="GPT-5.3-Codex-Spark",
                        ),
                        "additional_gpt_5_3_codex_spark_secondary_window": usage_module.UsageWindow(
    ...
    monkeypatch.setattr(cli_module, "fetch_all_usage", fake_fetch_all_usage)
    monkeypatch.setattr(cli_module.console, "_width", 260)

    default_result = runner.invoke(
        cli, ["list", "--no-interactive"], terminal_width=260
    )
    all_result = runner.invoke(
        cli, ["list", "--all", "--no-interactive"], terminal_width=260
    )

    assert default_result.exit_code == 0
    assert "Spark" not in default_result.output
    assert "12%" not in default_result.output
    assert "9%" not in default_result.output
    assert all_result.exit_code == 0
    assert "Spark" in all_result.output
    assert "12%" in all_result.output
    assert "9%" in all_result.output
...
def test_list_prompts_to_push_after_refresh(runner, saved_profile, monkeypatch, tmp_path):
    ...
    assert prompts == ["Refreshed stored tokens for profile work. Sync these changes now? [y/N]: "]
    assert push_calls == []
...
def test_list_prompts_only_once_when_reconcile_and_refresh_both_update_store(
    ...
    monkeypatch.setattr(cli_module, "fetch_all_usage", fake_fetch_all_usage)
    monkeypatch.setattr(cli_module, "interactive_prompt", lambda profiles: None)
    monkeypatch.setattr(cli_module, "_sync_now", lambda sync_dir_arg: None)
    ...
    assert prompts == ["List updated local store. Sync these changes now? [y/N]: "]
...
def test_import_newer_external_overwrites_without_prompt(
    ...
    external["tokens"]["access_token"] = "external-new-access"
    external["last_refresh"] = "2025-01-02T00:00:00+00:00"
    ...
    assert "Imported profile work" in result.output
    assert store_module.load_profile("work")["tokens"]["access_token"] == "external-new-access"
...
def test_import_mismatched_account_shows_metadata_and_can_skip(
    runner, sample_profile, monkeypatch, tmp_path
):
    ...
    assert result.exit_code == 0
    assert "Cannot automatically merge profile 'work'" in result.output
    assert "different accounts" in result.output
    assert "Replace local with external anyway?" in result.output
...
def test_export_newer_local_overwrites_without_prompt(
    ...
    external = json.loads(json.dumps(saved_profile))
    external["tokens"]["access_token"] = "old-access"
    external["last_refresh"] = "2024-12-31T00:00:00+00:00"
    existing.write_text(json.dumps(external))
...
def test_export_ambiguous_credentials_shows_metadata_and_can_confirm(
    runner, saved_profile, monkeypatch, tmp_path
):
    external_timestamp = 1_700_000_000
    local_timestamp = 1_600_000_000
    ...
    assert result.exit_code == 0
    assert "Cannot automatically merge profile 'work'" in result.output
    assert "Replace external with local anyway?" in result.output
    assert format_modified(datetime.fromtimestamp(local_timestamp)) in result.output
    assert format_modified(datetime.fromtimestamp(external_timestamp)) in result.output
    assert json.loads(existing.read_text())["auth_mode"] == "chatgpt"
...
def test_pull_updates_local_auth_from_newer_imported_active_profile(runner, monkeypatch, tmp_path):
    ...
    imported["last_refresh"] = "2025-01-02T00:00:00+00:00"
...
def test_pull_failure_surfaces_git_error(runner, monkeypatch, tmp_path):
    ...
    def fake_run(cmd, cwd, capture_output, text, check):
        ...
        if cmd == ["git", "pull", "--no-rebase", "--no-edit"]:
            return subprocess.CompletedProcess(
                cmd,
                1,
                stdout="CONFLICT (content): Merge conflict in work.json\n",
                stderr="From example.test:profiles\n",
            )
        raise AssertionError(f"Unexpected command: {cmd}")
    ...
    assert "From example.test:profiles" in result.output
    assert "CONFLICT (content): Merge conflict in work.json" in result.output
...
def test_push_no_changes_is_success(runner, monkeypatch, tmp_path):
    ...
    def fake_run(cmd, cwd, capture_output, text, check):
        ...
        if cmd == ["git", "diff", "--cached", "--quiet"]:
            return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
        if cmd == ["git", "push"]:
            return subprocess.CompletedProcess(cmd, 0, stdout="Everything up-to-date\n", stderr="")
        raise AssertionError(f"Unexpected command: {cmd}")
...
def test_push_preflight_pull_failure_does_not_export(runner, monkeypatch, tmp_path):
    sync_dir = tmp_path / "sync"
    sync_dir.mkdir()
    profile_path = sync_dir / "work.json"
    profile_path.write_text('{"sentinel": "unchanged"}')
    (tmp_path / ".env").write_text(f"CODEXAUTH_SYNC_DIR={sync_dir}\n")
    monkeypatch.chdir(tmp_path)
    store_module.save_profile(
        "work",
    ...
    assert result.exit_code != 0
    assert "From example.test:profiles" in result.output
    assert "CONFLICT (content)" in result.output
    assert "Exported profile work" not in result.output
    assert profile_path.read_text() == '{"sentinel": "unchanged"}'
    assert calls == [
        (["git", "rev-parse", "--is-inside-work-tree"], sync_dir),
        (["git", "pull", "--no-rebase", "--no-edit"], sync_dir),
    ]
